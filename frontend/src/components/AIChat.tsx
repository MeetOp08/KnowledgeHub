...
const AIChat: React.FC = () => {
  ...
  const userId = "student123"; // replace with auth userId
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/api/chat/history/${storedSession}?userId=${userId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.messages) {
          ...
          <div className="p-4 bg-gray-50 border-b">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {quickActions.map((action, i) => (
                <button key={i} onClick={() => handleQuickAction(action.text)}
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex items-start space-x-3 ${msg.type === "user" ? "flex-row-reverse space-x-reverse" : ""}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  msg.type === "user" ? "bg-gradient-to-r from-blue-500 to-indigo-600" : "bg-gradient-to-r from-green-500 to-emerald-600"}`}>
                  {msg.type === "user" ? <User className="h-4 w-4 text-white" /> : <Bot className="h-4 w-4 text-white" />}
                </div>
                <div className={`max-w-md p-4 rounded-2xl prose prose-sm max-w-none ${
