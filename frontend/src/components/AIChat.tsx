...
const AIChat: React.FC = () => {
  ...
  const loadChatSessions = async (uid: string) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/chat/sessions/${uid}`, {
        credentials: "include",
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const loadChatHistory = async (sessionIdToLoad: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/chat/history/${sessionIdToLoad}?userId=${userId}`,
        { credentials: "include" }
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const deleteSession = async (sessionIdToDelete: string, e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/chat/session/${sessionIdToDelete}?userId=${userId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
                  ...
                  <div className={`max-w-[80%] rounded-lg px-4 py-3 ${message.role === "user" ? "bg-purple-600 text-white" : "bg-white text-gray-800 border border-gray-200"}`}>
                    <div className="prose prose-sm max-w-none break-words">
                      <ReactMarkdown
                        ...
                        components={{
                          a: ({ node, ...props }) => <a {...props} className={`${message.role === "user" ? "text-blue-200 hover:text-blue-100" : "text-blue-600 hover:text-blue-800"} underline font-medium break-all`} target="_blank" rel="noopener noreferrer" />,
                          ...
                          h3: ({ node, ...props }) => <h3 {...props} className="text-md font-bold mb-2 mt-2" />,
                          code: ({ node, inline, className, children, ...props }: any) => {
                            return !inline ? (
                              <div className="bg-gray-800 rounded-md my-3 overflow-hidden border border-gray-700">
                                <div className="bg-gray-900 px-3 py-1.5 text-xs text-gray-400 border-b border-gray-700 flex justify-between items-center">
                                  <span>Code snippet</span>
                                </div>
                                <div className="p-3 overflow-x-auto text-gray-100 text-sm font-mono">
                                  <code className={className} {...props}>{children}</code>
                                </div>
                              </div>
                            ) : (
                              <code className={`${message.role === "user" ? "bg-purple-700 text-purple-100" : "bg-gray-100 text-pink-600"} rounded px-1.5 py-0.5 text-sm font-mono`} {...props}>{children}</code>
                            )
                          }
                        }}
                      >
